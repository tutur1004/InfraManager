...
public class PterodactylRequests extends HttpExecute {
    private static final String ENDPOINT = Main.getConfig().getString("host.pterodactyl.endpoint");
    private static final String ADMIN_KEY = Main.getConfig().getString("host.pterodactyl.admin.key");
    private static final String ACCOUNT_KEY = Main.getConfig().getString("host.pterodactyl.account.key");
    private static final String NODE = Main.getConfig().getString("host.pterodactyl.node");
        ...
        try {
            return execute(new URL(ENDPOINT + "/api/application/servers"), "POST", ADMIN_KEY,
                    new JSONObject().put("name", instance.getName())
                            ...
                            .put("user", Main.getConfig().getString("host.pterodactyl.account.id"))
                            .put("egg", Main.getConfig().getString("host.pterodactyl.egg"))
                            .put("docker_image", instance.getGame().getImage())
                            .put("startup", Main.getConfig().getString("host.settings.command")
    ...
    public static void setupStaff(String serverIdentifier) throws HostExecuteException {
        try {
            for (PterodactylUsersPermissions rank : PterodactylUsersPermissions.values()) {
                for (String mail : Main.getConfig().getStringList("host.pterodactyl.staff." + rank.name())) {
